...
exports.create = (req, res) => {
    ...
    activityOfficer.save().then(data => {
        ...
        res.status(500).send({
           message: err.message || "Some error occured while creating the Activity Officer." 
        });
    });
};
...
exports.findByCriteria = (req, res) => {
    ...
    ActivityOfficer.find(query).then(activityOfficers => {
        res.send(activityOfficers);
    }).catch(err => {
        res.status(500).send({
            message: err.message || "Some error occured while retrieving activity officers."
        });
    });
};
...
exports.update = (req, res) => {
    ...
    ActivityOfficer.findByIdAndUpdate(req.params.activityOfficerId, {
        position: req.body.position,
        activity: req.body.activity,
        employee: req.body.employee
    }, {new: true})
    .then(activity => {
        ...
        return res.status(500).send({
            message: "Error updating activity officer with id " + req.params.activityOfficerId
        });
    });
};
...
exports.delete = (req, res) => {
    ActivityOfficer.findByIdAndRemove(req.params.activityOfficerId)
    .then(activityOfficer => {
        if(!activityOfficer) {
            return res.status(404).send({
                message: "Activity officer not found with id " + req.params.activityOfficerId
            });
        ...
        return res.status(500).send({
            message: "Could not delete activity officer with id " + req.params.activityOfficerId
        });
    });
};